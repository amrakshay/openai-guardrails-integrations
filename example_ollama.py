...
client = OpenAI(
    base_url = os.getenv("OLLAMA_SERVER_BASE_URL"),
...
def detect_guardrails(input_text):
    messages = [
        {
...

            {
                ...
                Just give me json in the response, don't provide any other text.
                """
        },
        {
            "role": "user",
    ...
    try:
        ...
        response = client.chat.completions.create(
            model="tinyllama",  # or use "gpt-3.5-turbo" if needed
            messages=messages,
        ...
        # Define pricing (check OpenAI for current rates)
        prompt_cost_per_1000 = 0.03  # USD per 1,000 tokens for prompt
        completion_cost_per_1000 = 0.06  # USD per 1,000 tokens for completion
        ...
        print(f"Cost for this prompt: ${total_cost:.4f} USD")
